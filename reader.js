const mangaData = {
  Knightsoul_Vacation: {
    // 🎵 แยก BGM ตามช่วงหน้า พร้อมกำหนด volume แยกแต่ละเพลงได้
    bgmTracks: [
  ...
  Radical_Vacation: {
    bgmTracks: [
      { start: 2, end: 7, src: "asset/Radical Vacation/onsen.wav", volume: 0.5 },
...
// Preload รูปภาพและเสียงของหน้าถัดไป
...
// ระบบเช็คและสลับ BGM ตามช่วงหน้า (รองรับ volume แยกแต่ละเพลง)
function updateBGMForPage(pageNum) {
  ...
  if (track) {
    const bgmVolume = track.volume !== undefined ? track.volume : 0.5; // ค่าเริ่มต้นถ้าไม่ได้ระบุคือ 0.5
    ...
    if (currentBgmSrc !== track.src) {
      ...
      // หากเป็นเพลงเดิม แต่ช่วงนั้นกำหนดระดับเสียงต่างกัน ให้ปรับระดับเสียงทันที
      if (bgmAudio) {
        bgmAudio.volume = bgmVolume;
      }
    }
